 ...
 * Levenshtein Distance Algorithm implementation as MySQL UDF
  ...
  Compile the plugin :
    g++ -fPIC -o damlevlim256.so -shared damlevlim256.cpp -I /usr/include/mysql/
    (mac) g++ -bundle -o damlevlim256.so damlevlim256.cpp -I /usr/include/mysql/

  Get the plugin directory in mysql : 
     show variables like 'plugin_dir';

  Copy the library in the mysql plugin directory:
     sudo cp damlevlim256.so /usr/lib/mysql/plugin/

  Declare the function in mysql :
