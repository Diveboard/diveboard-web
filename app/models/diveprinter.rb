...
class Diveprinter
  ...
  handle_asynchronously :generate_pdf_dives
   ...
   def pdf_diveinfo p
     ...
     if !@dive.spot.location_name.nil?
       if spot_s != "" then spot_s += " - " end
       spot_s += begin @dive.spot.location_name.titleize rescue "" end
     end
     ...
     url = "https://maps.google.com/maps/api/staticmap?center=#{@dive.spot.lat.to_f},#{@dive.spot.long.to_f}&zoom=#{zoomfix}&size=#{map_w.to_i}x#{map_h.to_i}&maptype=terrain&markers=icon:https://www.diveboard.com/img/marker.png|#{@dive.spot.lat.to_f},#{@dive.spot.long.to_f}&sensor=false&format=png&key=#{@gmapskey}"
