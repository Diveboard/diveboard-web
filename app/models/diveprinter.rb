...
class Diveprinter
  ...
  handle_asynchronously :generate_pdf_dives
   ...
   def pdf_diveinfo p
     ...
     url = "https://maps.google.com/maps/api/staticmap?center=#{@dive.spot.lat.to_f},#{@dive.spot.long.to_f}&zoom=#{zoomfix}&size=#{map_w.to_i}x#{map_h.to_i}&maptype=terrain&markers=icon:https://www.diveboard.com/img/marker.+     Rails.logger.debug "Getting map at url #{url}"
